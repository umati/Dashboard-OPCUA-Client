...
class IDashboardDataClient {
 public:
  ...
  struct BrowseContext_t {
    ...
    inline static BrowseContext_t WithReference(ModelOpcUa::NodeId_t referenceTypeId) {
      BrowseContext_t ret;
    ...
    inline static BrowseContext_t ObjectAndVariableWithTypes() {
      BrowseContext_t ret;
      ret.nodeClassMask = (std::uint32_t)NodeClassMask::OBJECT | (std::uint32_t)NodeClassMask::OBJECT_TYPE | (std::uint32_t)NodeClassMask::VARIABLE |
    ...
    inline static BrowseContext_t Variable() {
      BrowseContext_t ret;
      ret.nodeClassMask = (std::uint32_t)NodeClassMask::VARIABLE | (std::uint32_t)NodeClassMask::VARIABLE_TYPE;
      ret.referenceTypeId = NodeId_HierarchicalReferences;
      return ret;
    }
  };

  virtual std::list<ModelOpcUa::BrowseResult_t> Browse(ModelOpcUa::NodeId_t startNode, BrowseContext_t browseContext) = 0;

  virtual bool isSameOrSubtype(const ModelOpcUa::NodeId_t &expectedType, const ModelOpcUa::NodeId_t &checkType, std::size_t maxDepth) = 0;

  virtual std::list<ModelOpcUa::BrowseResult_t> BrowseWithResultTypeFilter(
    ModelOpcUa::NodeId_t startNode, BrowseContext_t browseContext, ModelOpcUa::NodeId_t typeDefinition) = 0;

  // Deprecated
  inline virtual std::list<ModelOpcUa::BrowseResult_t> Browse(
    ModelOpcUa::NodeId_t startNode, ModelOpcUa::NodeId_t referenceTypeId, ModelOpcUa::NodeId_t typeDefinition) {
    return BrowseWithResultTypeFilter(startNode, BrowseContext_t::WithReference(referenceTypeId), typeDefinition);
  }

  // Deprecated
  inline virtual std::list<ModelOpcUa::BrowseResult_t> BrowseHasComponent(ModelOpcUa::NodeId_t startNode, ModelOpcUa::NodeId_t typeDefinition) {
  ...
  virtual ModelOpcUa::NodeId_t TranslateBrowsePathToNodeId(ModelOpcUa::NodeId_t startNode, ModelOpcUa::QualifiedName_t browseName) = 0;

  std::map<std::string, uint16_t> m_uriToIndexCache;

  class ValueSubscriptionHandle {
   public:
    ValueSubscriptionHandle(int32_t clientHandle, int32_t monItemId, ModelOpcUa::NodeId_t nodeId)
      : m_clientHandle(clientHandle), m_monitoredItemId(monItemId), m_nodeId(nodeId) {}
   ...
   protected:
    void setUnsubscribed() { m_unsubscribed = true; }
