...
class DashboardClient {
 ...
 protected:
  ...
  std::map<ModelOpcUa::NodeId_t, std::list<std::shared_ptr<const ModelOpcUa::Node>>> browsedNodesChildNodes;
  ...
  void handleSubscribeChildNodes(
    const std::shared_ptr<const ModelOpcUa::SimpleNode> &pNode,
    std::map<std::shared_ptr<const ModelOpcUa::Node>, nlohmann::json> &valueMap,
    std::mutex &valueMap_mutex);

  void handleSubscribePlaceholderChildNode(
  ...
  std::shared_ptr<DataSetStorage_t> prepareDataSetStorage(
    const ModelOpcUa::NodeId_t &startNodeId,
    const std::shared_ptr<ModelOpcUa::StructureNode> &pTypeDefinition,
    const std::string &channel,
    const std::string &onlineChannel);

  bool OptionalAndMandatoryTransformToNodeId(
  ...
  bool OptionalAndMandatoryPlaceholderTransformToNodeId(
    const ModelOpcUa::NodeId_t &startNode,
    std::list<std::shared_ptr<const ModelOpcUa::Node>> &foundChildNodes,
    const std::shared_ptr<ModelOpcUa::StructureNode> &pChild);

  void TransformToNodeIdNodeNotFoundLog(const ModelOpcUa::NodeId_t &startNode, const std::shared_ptr<ModelOpcUa::StructureNode> &pChild) const;
};
}  // namespace Dashboard
}  // namespace Umati
