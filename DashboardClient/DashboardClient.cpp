...
DashboardClient::DashboardClient(
  std::shared_ptr<IDashboardDataClient> pDashboardDataClient, std::shared_ptr<IPublisher> pPublisher, std::shared_ptr<OpcUaTypeReader> pTypeReader)
...
void DashboardClient::addDataSet(
  ...
  try {
    std::shared_ptr<DataSetStorage_t> pDataSetStorage = prepareDataSetStorage(startNodeId, pTypeDefinition, channel, onlineChannel);
...
std::shared_ptr<DashboardClient::DataSetStorage_t> DashboardClient::prepareDataSetStorage(
  const ModelOpcUa::NodeId_t &startNodeId,
  const std::shared_ptr<ModelOpcUa::StructureNode> &pTypeDefinition,
  const std::string &channel,
  const std::string &onlineChannel) {
...
std::string DashboardClient::getJson(const std::shared_ptr<DataSetStorage_t> &pDataSetStorage) {
  auto getValueCallback = [pDataSetStorage](const std::shared_ptr<const ModelOpcUa::Node> &pNode) -> nlohmann::json {
    ...
    if (it == pDataSetStorage->values.end()) {
      LOG(DEBUG) << "Couldn't write value for " << pNode->SpecifiedBrowseName.Name << " | " << pNode->SpecifiedTypeNodeId.Uri << ";"
                 << pNode->SpecifiedTypeNodeId.Id << "Try to search it with NodeId!";
      ...
      if (pSimpleNode) {
        ...
        for (auto it1 : values) {
          ...
          if (pSimpleNode1->NodeId == pSimpleNode->NodeId) {
            ...
            LOG(DEBUG) << pNode->SpecifiedBrowseName.Name << " "
                       << "found!";
            return it1.second;
          }
        }
...
void LogOptionalAndMandatoryTransformToNodeIdError(const ModelOpcUa::NodeId_t &nodeId, const ModelOpcUa::QualifiedName_t &childBrowsName, const char *err) {
  LOG(ERROR) << "Forwarding exception, cause:"
             << "Could not find '" << static_cast<std::string>(nodeId) << "'->'" << static_cast<std::string>(childBrowsName) << "'"
             << "Unknown ID caused exception: " << err;
}

std::string GetOptionalAndMandatoryTransformToNodeIdError(
  const ModelOpcUa::NodeId_t &nodeId, const ModelOpcUa::QualifiedName_t &childBrowseName, const char *err) {
  return "In '" + static_cast<std::string>(nodeId) + "'->'" + static_cast<std::string>(childBrowseName) + "':\n" + err;
}

/**
 * @return if the switch case should break
 */
bool DashboardClient::OptionalAndMandatoryTransformToNodeId(
...
void DashboardClient::TransformToNodeIdNodeNotFoundLog(const ModelOpcUa::NodeId_t &startNode, const std::shared_ptr<ModelOpcUa::StructureNode> &pChild) const {
  LOG(INFO) << "Could not find '" << static_cast<std::string>(startNode) << "'->'" << static_cast<std::string>(pChild->SpecifiedBrowseName) << "'";
}

bool DashboardClient::OptionalAndMandatoryPlaceholderTransformToNodeId(
  const ModelOpcUa::NodeId_t &startNode,
  std::list<std::shared_ptr<const ModelOpcUa::Node>> &foundChildNodes,
  const std::shared_ptr<ModelOpcUa::StructureNode> &pChild) {
  try {
    ...
    if (!pPlaceholderChild) {
      LOG(ERROR) << "Child " << pChild->SpecifiedBrowseName.Uri << ";" << pChild->SpecifiedBrowseName.Name << " of " << startNode.Uri << ";" << startNode.Id
    ...
    if (pChild->ModellingRule != ModelOpcUa::ModellingRule_t::OptionalPlaceholder) {
      LOG(ERROR) << "Forwarding exception, cause:"
                 << "Could not find '" << static_cast<std::string>(startNode) << "'->'" << static_cast<std::string>(pChild->SpecifiedBrowseName) << "'"
...
std::shared_ptr<const ModelOpcUa::PlaceholderNode> DashboardClient::BrowsePlaceholder(
  ModelOpcUa::NodeId_t startNode, std::shared_ptr<const ModelOpcUa::StructurePlaceholderNode> pStructurePlaceholder) {
  ...
  auto pPlaceholderNode = std::make_shared<ModelOpcUa::PlaceholderNode>(*pStructurePlaceholder, std::list<std::shared_ptr<const ModelOpcUa::Node>>{});
...
void DashboardClient::preparePlaceholderNodesTypeId(
  ...
  for (auto &browseResult : browseResults) {
    if (browseResult.TypeDefinition.Id == NodeId_BaseObjectType.Id) {
      LOG(INFO) << "TypeDefinition is BaseObjectType " << static_cast<std::string>(browseResult.NodeId) << "- Try to find an Interface";
      auto ifs = m_pDashboardDataClient->Browse(browseResult.NodeId, Dashboard::IDashboardDataClient::BrowseContext_t::HasInterface());
      if (ifs.empty()) {
...
void DashboardClient::handleSubscribeChildNodes(
  const std::shared_ptr<const ModelOpcUa::SimpleNode> &pNode,
  std::map<std::shared_ptr<const ModelOpcUa::Node>, nlohmann::json> &valueMap,
  std::mutex &valueMap_mutex) {
...
void DashboardClient::handleSubscribeChildNode(
  const std::shared_ptr<const ModelOpcUa::Node> &pChildNode,
  std::map<std::shared_ptr<const ModelOpcUa::Node>, nlohmann::json> &valueMap,
  std::mutex &valueMap_mutex) {
...
void DashboardClient::handleSubscribePlaceholderChildNode(
  const std::shared_ptr<const ModelOpcUa::Node> &pChildNode,
  std::map<std::shared_ptr<const ModelOpcUa::Node>, nlohmann::json> &valueMap,
  std::mutex &valueMap_mutex) {
...
void DashboardClient::subscribeValue(
  const std::shared_ptr<const ModelOpcUa::SimpleNode> &pNode,
  std::map<std::shared_ptr<const ModelOpcUa::Node>, nlohmann::json> &valueMap,
  std::mutex &valueMap_mutex) { /**
                                 ...
                                 */
  // LOG(INFO) << "SubscribeValue " << pNode->SpecifiedBrowseName.Uri << ";" << pNode->SpecifiedBrowseName.Name << " | " << pNode->NodeId.Uri << ";" <<
  // pNode->NodeId.Id;
  ...
  try {
    for (auto value : m_subscribedValues) {
      if (value && value.get()->getNodeId() == pNode.get()->NodeId) return;
    }
