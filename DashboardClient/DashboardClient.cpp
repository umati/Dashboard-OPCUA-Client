			...
			for (auto &pChild : *pTypeDefinition->SpecifiedChildNodes) {
				switch (pChild->ModellingRule) {
					...
					case ModelOpcUa::ModellingRule_t::Mandatory: {
						bool should_continue = OptionalAndMandatoryTransformToNodeId(startNode, foundChildNodes,
																					 pChild);
						if (should_continue) {
							continue;
						}
					...
					case ModelOpcUa::ModellingRule_t::MandatoryPlaceholder: {
						bool should_continue = OptionalAndMandatoryPlaceholderTransformToNodeId(startNode,
																								foundChildNodes,
																								pChild);
						if (should_continue) {
							continue;
