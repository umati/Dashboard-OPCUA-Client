  ...
  switch (pNode->ModellingRule) {
    ...
    case ModelOpcUa::ModellingRule_t::OptionalPlaceholder: {
      ...
      for (const auto &pPlaceholderElement : placeholderElements) {
        auto el = (ModelToJson(pPlaceholderElement.pNode, getValue, serializeNodeInformation, nestAsChildren, publishNullValues).getJson());
...
bool ModelToJson::isBaseDataVariableType(const std::shared_ptr<const ModelOpcUa::SimpleNode> &pSimpleNode) {
  LOG(INFO) << "check " << pSimpleNode->NodeId.Uri << pSimpleNode->NodeId.Id;
  auto ret =
