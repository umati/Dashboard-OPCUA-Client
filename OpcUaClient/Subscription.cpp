...
static void createDataChangeCallback(UA_Client *client, UA_UInt32 subId, void *subContext, UA_UInt32 monId, void *monContext, UA_DataValue *dataValue) {
...
Subscription::Subscription(const std::map<std::string, uint16_t> &uriToIndexCache, const std::map<uint16_t, std::string> &indexToUriCache)
  : m_uriToIndexCache(uriToIndexCache), m_indexToUriCache(indexToUriCache) {
  LOG(WARNING) << "Created subscription " << this;
}

void Subscription::setSubscriptionWrapper(OpcUaSubscriptionInterface *pSubscriptionWrapper) { m_pSubscriptionWrapper = pSubscriptionWrapper; }
...
void Subscription::dataChange(
  UA_Int32 /*clientSubscriptionHandle*/, const UA_DataChangeNotification &dataNotifications, const UA_DiagnosticInfo & /*diagnosticInfos*/) {
  ...
  for (UA_Int32 i = 0; i < dataNotifications.monitoredItemsSize; ++i) {
    ...
    auto value = Converter::UaDataValueToJsonValue(UA_DataValue(dataNotifications.monitoredItems->value), false).getValue();
    it->second(value);
  }
}

void Subscription::newEvents(UA_Int32 clientSubscriptionHandle, UA_EventFieldList &eventFieldList) { LOG(ERROR) << "Received new Event, Not implemented."; }

void Subscription::createSubscription(UA_Client *client) {
  auto request = UA_CreateSubscriptionRequest_default();
  auto result = m_pSubscriptionWrapper->SessionCreateSubscription(client, request, this, NULL, NULL);
...
void Subscription::deleteSubscription(UA_Client *client) { m_pSubscriptionWrapper->SessionDeleteSubscription(client, m_pSubscriptionID); }
...
std::shared_ptr<Dashboard::IDashboardDataClient::ValueSubscriptionHandle> Subscription::Subscribe(
  UA_Client *client, ModelOpcUa::NodeId_t nodeId, Dashboard::IDashboardDataClient::newValueCallbackFunction_t callback) {
  ...
  try {
    monItemCreateResult = UA_Client_MonitoredItems_createDataChange(
      client,
      m_pSubscriptionID,
      UA_TIMESTAMPSTORETURN_SOURCE,
      monItemCreateReq,
      (void *)((UA_Int64)(monItemCreateReq.requestedParameters.clientHandle)),
      createDataChangeCallback,
      NULL);
    ...
    auto returnPointer = std::make_shared<Dashboard::IDashboardDataClient::ValueSubscriptionHandle>(
      monItemCreateResult.monitoredItemId, monItemCreateReq.requestedParameters.clientHandle, nodeId);
...
UA_MonitoredItemCreateRequest &Subscription::prepareMonItemCreateReq(
  const ModelOpcUa::NodeId_t &nodeId, UA_MonitoredItemCreateRequest &monItemCreateReq) const {
  ...
  open62541Cpp::UA_NodeId id = (open62541Cpp::UA_NodeId)(Converter::ModelNodeIdToUaNodeId(nodeId, m_uriToIndexCache).getNodeId());
  UA_NodeId_copy(id.NodeId, &monItemCreateReq.itemToMonitor.nodeId);

  return monItemCreateReq;
}

void Subscription::validateMonitorItemResult(
  const UA_StatusCode &uaResult, UA_MonitoredItemCreateResult monItemCreateResult, const ModelOpcUa::NodeId_t &nodeId) {
  if (UA_StatusCode_isBad(uaResult)) {
    LOG(ERROR) << "Create Monitored items for " << nodeId.Uri << ";" << nodeId.Id << " failed with: " << UA_StatusCode_name(uaResult);
    throw Exceptions::OpcUaNonGoodStatusCodeException(uaResult);
  }

  if (monItemCreateResult.revisedQueueSize != 1) {
    LOG(ERROR) << "Expect monItemCreateResult.length() == 1 for " << nodeId.Uri << ";" << nodeId.Uri << " , got:" << monItemCreateResult.revisedQueueSize;
    throw Exceptions::UmatiException("Length mismatch.");
  }
  if (UA_StatusCode_isBad(monItemCreateResult.statusCode)) {
    LOG(ERROR) << "Monitored Item status code bad for " << nodeId.Uri << ";" << nodeId.Uri << " : " << monItemCreateResult.statusCode;
  }
}
}  // namespace OpcUa
}  // namespace Umati
