...
void OpcUaClient::readTypeDictionaries() {
  ...
  for (auto &dict : dictionaryResults) {
    ...
    for (XMLIterator it{0, firstNode}; it.current != nullptr || (it.current != firstNode && it.visitedElements != 0);
      ...
      if (eResult != tinyxml2::XML_SUCCESS) continue;
      ...
      for (XMLIterator it2 = XMLIterator(0, firstNodeInner); it2.current != nullptr || (it2.current != firstNodeInner && it2.visitedElements != 0);
        ...
        if (eResult == tinyxml2::XML_SUCCESS) field.TypeName = std::string(buffer);
        eResult = it2.current->QueryStringAttribute("Name", &buffer);
        if (eResult == tinyxml2::XML_SUCCESS) field.Name = std::string(buffer);
        eResult = it2.current->QueryStringAttribute("LengthField", &buffer);
        if (eResult == tinyxml2::XML_SUCCESS) field.LengthField = std::string(buffer);
        eResult = it2.current->QueryStringAttribute("SwitchField", &buffer);
        if (eResult == tinyxml2::XML_SUCCESS) field.SwitchField = std::string(buffer);
        eResult = it2.current->QueryStringAttribute("Terminator", &buffer);
        if (eResult == tinyxml2::XML_SUCCESS) field.Terminator = std::string(buffer);
    ...
    for (XMLIterator it{0, firstNode}; it.current != nullptr || (it.current != firstNode && it.visitedElements != 0);
      ...
      for (XMLIterator it2{0, firstNodeInner}; it2.current != nullptr || (it2.current != firstNodeInner && it2.visitedElements != 0);
