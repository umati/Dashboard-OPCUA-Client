...
namespace OpcUa {
	...
	bool OpcUaClient::connect(std::string serverURI)
	{
		...
		if (result.isBad())
		{
			std::cout << "[OPC UA DataClient] could not get Endpoints.(" << result.toString().toUtf8() << ")" << __FILE__ << ":" << __LINE__ << std::endl;
			return false;
		}
		...
		for (OpcUa_UInt32 iEndpoint = 0; iEndpoint < endpointDescriptions.length(); iEndpoint++)
		{
			...
			std::cout << "desiredEnpoint.url: " << desiredEnpoint.url.toUtf8() << std::endl;
		...
		{
			std::cout << "[OPC UA DataClient] could not find endpoint without encryption." << __FILE__ << ":" << __LINE__ << std::endl;
			return false;
		}
		...
		if (!result.isGood())
		{
			std::cout << "Connecting failed in OPC UA Data Client: " << result.toString().toUtf8() << std::endl;
			return false;
		}
	...
	void OpcUaClient::connectionStatusChanged(OpcUa_UInt32 clientConnectionId, UaClientSdk::UaClient::ServerStatus serverStatus)
	{
		...
		case UaClientSdk::UaClient::Disconnected:
			LOG(ERROR) << "[OPC UA DataClient] Disconnected." << std::endl;
			m_isConnected = false;
			break;
		case UaClientSdk::UaClient::Connected:
			LOG(ERROR) << "[OPC UA DataClient] Connected." << std::endl;
			m_isConnected = true;
			break;
		case UaClientSdk::UaClient::ConnectionWarningWatchdogTimeout:
			LOG(ERROR) << "[OPC UA DataClient] ConnectionWarningWatchdogTimeout." << std::endl;
			break;
		case UaClientSdk::UaClient::ConnectionErrorApiReconnect:
			LOG(ERROR) << "[OPC UA DataClient] ConnectionErrorApiReconnect." << std::endl;
			break;
		case UaClientSdk::UaClient::ServerShutdown:
			LOG(ERROR) << "[OPC UA DataClient] ServerShutdown." << std::endl;
			break;
		case UaClientSdk::UaClient::NewSessionCreated:
			LOG(ERROR) << "[OPC UA DataClient] NewSessionCreated." << std::endl;
			break;
		}
	}
