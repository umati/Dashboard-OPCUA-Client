...
namespace Umati {
    ...
    namespace OpcUa {
        ...
        auto startVariableType = handleBrowseTypeResult(bidirectionalTypeMap, startBrowseTypeResult, nullptr,
                                                        ModelOpcUa::ModellingRule_t::Mandatory, ofBaseDataVariableType);
    ...
    void OpcUaClient::findObjectTypeNamespacesAndCreateTypeMap(std::vector <std::string> &notFoundObjectTypeNamespaces,
                                                               size_t i,
...
OpcUaClient::ReferenceDescriptionToBrowseResult(const OpcUa_ReferenceDescription &referenceDescription) {
    ...
    entry.NodeId = Converter::UaNodeIdToModelNodeId(UaNodeId(UaExpandedNodeId(referenceDescription.NodeId).nodeId()),
...
ModelOpcUa::NodeId_t
OpcUaClient::TranslateBrowsePathToNodeId(ModelOpcUa::NodeId_t startNode, ModelOpcUa::QualifiedName_t browseName) {
    checkConnection();
    ...
    if (uaResultElement.isBad()) {
        LOG(ERROR) << "Element returned bad status code: " << uaResultElement.toString().toUtf8() << " for node: '"
    ...
    if (uaBrowsePathResults[0].NoOfTargets != 1) {
        LOG(WARNING) << "Continuing with index 0 - expected one target, got " << uaBrowsePathResults[0].NoOfTargets
...
void OpcUaClient::createTypeMap(std::shared_ptr < std::map < std::string, std::shared_ptr < ModelOpcUa::StructureBiNode
...
bloodlineStringStream << "->" << ancestor->structureNode->SpecifiedBrowseName.Uri << ";" << ancestor->structureNode->SpecifiedBrowseName.
Name;
for(
...
Name &&
        fIt
.operator*()->SpecifiedBrowseName.Uri == structureNode->SpecifiedBrowseName.Uri
) {
findIterator = fIt;
found = true;
break;
...
void OpcUaClient::updateResultContainer(const std::string &inputString, std::vector <std::string> &resultContainer,
