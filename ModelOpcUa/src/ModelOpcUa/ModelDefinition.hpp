...
struct NodeId_t {
  ...
  bool isNull() const { return Uri.empty() && Id.empty(); }
...
struct QualifiedName_t {
  ...
  bool isNull() const { return Uri.empty() && Name.empty(); }
  ...
  bool operator==(const QualifiedName_t &other) const { return this->Uri == other.Uri && this->Name == other.Name; }
};
...
static const std::string ModellingRuleToString(const ModellingRule_t rule) {
  static std::vector<std::string> modellingRules = {"None", "Optional", "Madatory", "OptionalPlaceholder", "MandatoryPlacehoder"};
...
class StructureNode : public NodeDefinition {
 public:
  ...
  static void printYamlIntern(const std::shared_ptr<StructureNode> &node, const std::string &parentTree, int tabs, std::ostream &ss);
};
